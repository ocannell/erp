...
`pipeline/publish.sh` is the cron entry point. It rebuilds, **validates, and
only then deploys** — every failure path leaves the previously deployed file
byte-for-byte intact, because a stale chart is bad but a wrong chart is worse.

```
--daily   refresh Treasury 10y / ^GSPC / ^TNX and rebuild (cheap)
--full    also re-scan the Internet Archive for newly published S&P DJI
          workbook vintages, then reparse (slow, throttled)
```

Gates in order: market fetch → build → `validate.py` (4/4 or abort) →
`headless_render.js` (or abort) → payload shape check (>5,000 rows, expected
schema) → atomic `cp`+`mv` deploy. Verified by dry run: a simulated data
regression exited 1, logged `validation FAILED; previous web output preserved`,
and left the deployed md5 unchanged.

Install:

```bash
sudo mkdir -p /home/allofthesewords/erp_pipeline/logs
sudo cp -r pipeline tests data /home/allofthesewords/erp_pipeline/
sudo chmod +x /home/allofthesewords/erp_pipeline/pipeline/publish.sh
```

Cron (replaces the old `scripts/erp-update.py` entry, which builds a *trailing*
earnings ERP in an incompatible `{"data":[...]}` format and would overwrite
these outputs):

```cron
20 7 * * *   /home/allofthesewords/erp_pipeline/pipeline/publish.sh --daily >/dev/null 2>&1
40 7 * * 6   /home/allofthesewords/erp_pipeline/pipeline/publish.sh --full  >/dev/null 2>&1
```
